...
```
Pod 台数 × MaxOpened <= DB セッション上限（10,000 / インスタンス）
```

**計算例**（API Pod 20 台、ワーカー Pod 10 台の場合）:

- API: `20 × 400 = 8,000`
- ワーカー: `10 × 200 = 2,000`
- **合計 10,000**（上限ギリギリなのでスパイク時にさらに余裕が欲しい → MaxOpened を下げるか Pod 台数上限を監視）

スパイク時の HPA で Pod が 1.5 倍になる前提で、平常時は上限の **60〜70% に収める**。
